...
public class UpdateHandlerTest extends AbstractTestBase {
    ...
    public void handleRequest() {
        ...
        doReturn(staticIpHandler)
                .when(updateHandler).getStaticIpHandler(any(), any(), any(), any(), any());
        doReturn(readHandler)
                .when(updateHandler).getReadHandler();

        when(staticIpHandler.handleUpdate(any()))
        ...
        verify(staticIpHandler, times(1)).handleUpdate(ProgressEvent.progress(model, callbackContext));
