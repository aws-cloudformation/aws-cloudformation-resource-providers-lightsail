...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    public void handleRequest_SimpleSuccess() {
        ...
        doReturn(alarmHandler)
                .when(createHandler).getAlarmHandler(any(), any(), any(), any(), any());
        doReturn(readHandler)
                .when(createHandler).getReadHandler();

        when(alarmHandler.handleCreate(any()))
        ...
        verify(alarmHandler, times(1)).handleCreate(ProgressEvent.progress(model, callbackContext));
