...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    public void handleRequest_SimpleSuccess() {
        ...
        doReturn(bucketHandler)
                .when(createHandler).getBucketHandler(any(), any(), any(), any(), any());
        doReturn(updateHandler)
                .when(createHandler).getUpdateHandler();

        when(bucketHandler.handleCreate(any()))
        ...
        verify(bucketHandler, times(1)).handleCreate(ProgressEvent.progress(model, callbackContext));
