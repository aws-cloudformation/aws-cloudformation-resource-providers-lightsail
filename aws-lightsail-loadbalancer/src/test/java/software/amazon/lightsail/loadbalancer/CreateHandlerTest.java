...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    public void handleRequest_SimpleSuccess() {
        ...
        doReturn(loadBalancerHandler)
                .when(createHandler).getLoadBalancerHandler(any(), any(), any(), any(), any());
        doReturn(updateHandler)
                .when(createHandler).getUpdateHandler();

        when(loadBalancerHandler.handleCreate(any()))
        ...
        verify(loadBalancerHandler, times(1)).handleCreate(ProgressEvent.progress(model, callbackContext));
